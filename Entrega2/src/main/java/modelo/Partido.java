...
public class Partido{
	...
	public int getGanador() throws ErrorEnCargaDeDatos {
	    String aciertosEquipo1 = this.getEquipo1().getAciertos();		//obtengo los aciertos del equipo 1 (si hay X o no) en el archivo pronostico
	    String aciertosEquipo2 = this.getEquipo2().getAciertos();		//obtengo los aciertos del equipo 1 (si hay X o no)	en el archivo pronostico
	    ...
	    if (aciertosEquipo1 != null && aciertosEquipo2 != null) {	//condicional por si se llegaran a mesclar los datos
		    ...
		    if (aciertosEquipo1.equals("X")) {						//si se aposto por el equipo1 se retorna 1
		    	...
		    	ganador = 3;
		    } else if (aciertosEquipo1.equals(" ") && aciertosEquipo2.equals(" ")) {	//si se aposto por empate retorno 3
		    	ganador =  2;
		    } else {
		    	throw new ErrorEnCargaDeDatos("Error en la carga de datos de pronostico \nse esperaba una X");	//en caso que no se apostara lanzo error
		    }
	    ...
	    if (aciertosEquipo1 == null && aciertosEquipo2 == null) {		//condicional por si se llegaran a mesclar los datos
	    	...
	    	if(golesEquipo1>golesEquipo2) {				//si el equipo 1 gano retorno 1
	    		...
	    		throw new ErrorEnCargaDeDatos("Error en la carga de datos de resultado \nse esperaban valores numericos");	//en caso que haya error con valores en goles 
	    	}
