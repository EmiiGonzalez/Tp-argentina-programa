...
public class Resultado {
	...
	private void setPuntajeFinalLista() { /*
		...
		for (Persona p : this.getApuestasResultado()) { // itero la lista con el resultado de cada partido por persona
			...
			if (getPersonasPorFase().containsKey(faseId)) {		//si el map que contiene las fases contiene una que haga match con el faseId
				Fase f = getPersonasPorFase().get(faseId);		//invoco al objeto Fase utilizando ese id
				
				if (f.getPuntajeFinalLista().containsKey(id)) { // si el atributo map del objeto fase que contiene personas contiene el id de la persona iterada
			...
			} else {	//si el objeto fase no se encuentra en la lista (util para cuando solo hay una persona)
				Fase fa = new Fase();	//creo uno nuevo
				fa.getPuntajeFinalLista().put(id, p);	//al atributo del objeto fase le asigno una persona nueva
				getPersonasPorFase().put(faseId, fa);	//agrego el objeto fase a la lista de personas por fase
